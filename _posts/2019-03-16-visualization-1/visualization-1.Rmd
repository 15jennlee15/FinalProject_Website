...
I converted Group to a factor:

HRV1$Group <- as.factor(HRV1$Group)

I also recoded the labels of HRV Segment:
...
I created a second version of the final visual - the first shows the full range of the y-axis and the second is a zoomed in version that looks at a more limited range. I did this by adding "ylim(5.5,7.5)" to the code. In the first graph you'll notice there is a lot of white space because there are no data points in the lower range, which is not visually appealing.  The benefit of this however, is that it more accurately shows the scale of the axis and doesn't exaggerate differences between groups. The zoomed in version allows you to see more of the varaition between the groups which may or may not be significantly different from each other.